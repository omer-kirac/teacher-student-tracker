CREATE OR REPLACE FUNCTION get_class_rankings_weekly(teacher_id_input UUID)
RETURNS TABLE (
  class_id UUID,
  class_name TEXT,
...
BEGIN
  ...
  LEFT JOIN 
    student_solutions ss ON ss.student_id = s.id AND ss.date >= (CURRENT_DATE - INTERVAL '7 days')
  WHERE 
